...
# -----------------------------------------------------------------------------------------
FILE_NAME = 'EN_cura_out.gcode'#'EN_cube_out.gcode'      # filename including extension
...
def replace_z(x_val,y_val,z_val,e_val):
    x,y= x_val-PLATE_X,y_val-PLATE_Y
    if CONE_TYPE == 'outward':
        z_val = z_val - (math.tan(math.radians(CONE_ANGLE)) * math.sqrt(x**2+y**2))
    elif CONE_TYPE == 'inward':
        z_val = z_val + (math.tan(math.radians(CONE_ANGLE)) * math.sqrt(x**2+y**2))
    
    row_new = "G1"+" X"+str(round(x_val+X_MOVE,3))+" Y"+str(round(y_val+Y_MOVE,3))+" Z"+str(round(z_val+FIRST_LAYER_HEIGHT,3))+" E"+str(e_val)+"\n"
    return row_new
...
def main(path):
    ...
    path_write = re.sub(r'.gcode','_bt_.gcode', path_write)
    print(path_write)
